...
class ORMAuditManager extends BaseObjectAuditManager
{
    ...
    public function __construct(ObjectAuditConfiguration $configuration,
        ...
        Assert::isInstanceOf($defaultPersistManager, EntityManagerInterface::class);
        ...
        $this->auditConnection = $auditPersistManager->getConnection();
        $this->auditPlatform = $this->auditConnection->getDatabasePlatform();

        parent::__construct($configuration, $revisionFactory, $objectAuditFactory, $revisionRepository, $defaultPersistManager, $auditPersistManager);
    }
    ...
    public function findObjectByRevision(string $className, $objectIds, RevisionInterface $revision, ObjectManager $persistManager = null, array $options = array())
    {
        ...
        if ($persistManager === null) {
            $persistManager = $this->defaultPersistManager;
        }

        /** @var ClassMetadata $classMetadata */
        $classMetadata = $persistManager->getClassMetadata($className);
        $tableName = $this->getAuditTableNameForClass($className, $this->auditPersistManager);
        ...
        if ($classMetadata->isInheritanceTypeJoined() && $classMetadata->name != $classMetadata->rootEntityName) {
            ...
            $rootClass = $persistManager->getClassMetadata($classMetadata->rootEntityName);
            $rootTableName = $this->getAuditTableNameForClass($rootClass->name, $persistManager);
            $condition = array();
        ...
        if (!$row) {
            throw ObjectNotFoundException::forObjectAtSpecificRevision($classMetadata->name, $objectIds, $revision);
        }
        ...
        if ($options['threatDeletionsAsExceptions'] && $revisionTypeValue == RevisionInterface::ACTION_DELETE) {
            throw ObjectDeletedException::forObjectAtSpecificRevision($classMetadata->name, $objectIds, $revision);
        }

        unset($row[$fieldRevisionTypeName]);

        return $this->objectAuditFactory->createNewAudit($classMetadata->name, $columnMap, $row, $revision, $this, $persistManager);
    }
    ...
    public function findObjectsByFieldsAndRevision(string $className, array $fields, string $indexBy = null, RevisionInterface $revision, ObjectManager $persistManager = null, array $options = array()): array
    {
        ...
        foreach ($result as $identifiers) {
            ...
            $entity = $this->findObjectByRevision($className, $identifiers, $revision, $persistManager, $options);
            if ($key !== null) {
                $entities[$key] = $entity;
            } else {
                $entities[] = $entity;
    ...
    public function findObjectsChangedAtRevision(string $className, RevisionInterface $revision, ObjectManager $persistManager = null, array $options = array()): array
    {
        ...
        /** @var EntityManagerInterface $persistManager */
        if ($persistManager === null) {
            $persistManager = $this->getDefaultPersistManager();
        }
    ...
    public function findObjectRevisions(string $className, $objectIds, ObjectManager $persistManager = null): Collection
    {
        ...
        $revisionMetadata = $auditPersistManager->getClassMetadata($revisionClass);
        $revisionTableName = $revisionMetadata->getTableName();

        /** @var ClassMetadata $classMetadata */
        $classMetadata = $persistManager->getClassMetadata($className);
        $entityTableName = $this->getAuditTableNameForClass($className, $persistManager);
        ...
        foreach ($revisionMetadata->identifier as $revisionIdField) {
            if (isset($revisionMetadata->fieldMappings[$revisionIdField])) {
                ...
                $columnName = $revisionMetadata->associationMappings[$revisionIdField]['joinColumns'][0]['name'];
            } else {
                continue;
            }
        ...
        $rsm = new ResultSetMappingBuilder($auditPersistManager);
        $rsm->addRootEntityFromClassMetadata($revisionClass, 'r');
        $query = $auditPersistManager->createNativeQuery($queryBuilder->getSQL(), $rsm);
    ...
    public function getObjectHistory(string $className, $objectIds, ObjectManager $persistManager = null, array $options = array()): array
    {
        ...
        while ($row = $stmt->fetch(Query::HYDRATE_ARRAY)) {
            ...
            $revision = $this->revisionRepository->find($revisionIdentifiers);
    ...
    {
        if ($persistManager === null) {
            $persistManager = $this->auditPersistManager;
        }

        return $this->getObjectRevision($className, $objectIds, $persistManager);
    }
    ...
    public function getCurrentObjectRevision(string $className, $objectIds, ObjectManager $persistManager = null)
    {
        ...
        return $this->getObjectRevision($className, $objectIds, $persistManager, 'DESC');
    }
    ...
    public function saveObjectRevisionData(ChangedObject $changedObject)
    {
        ...
        $identifiers = $changedObject->getIdentifiers();

        /** @var EntityManagerInterface $persistManager */
        $persistManager = $changedObject->getPersistManager();
        $classMetadata = $persistManager->getClassMetadata($className);
     ...
     * @return string
     */
    public function getAuditTableNameForClass(string $className, ObjectManager $persistManager = null)
    {
        if ($persistManager === null) {
            $persistManager = $this->defaultPersistManager;
        }
    ...
    public function saveCurrentRevision()
    {
        if ($this->currentRevision !== null) {
            ...
            $revisionMetadata = $auditPersistManager->getClassMetadata($revisionName);
    ...
    protected function getInsertRevisionSQL(ClassMetadata $classMetadata)
    {
        if (!isset($this->insertRevisionSQL[$classMetadata->name])) {
            $placeholders = array('?', '?');
            $auditTableName = $this->getAuditTableNameForClass($classMetadata->name, $this->auditPersistManager);
            /** @var ORMAuditConfiguration $configuration */
            $configuration = $this->configuration;
